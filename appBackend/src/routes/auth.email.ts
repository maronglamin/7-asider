...
router.post('/register-email', async (req: Request, res: Response) => {
  try {
    ...
    const existing = await prisma.user.findFirst({ where: { email, NOT: { status: 'TERMINATED' as any } } as any });
    if (existing) return res.status(409).json({ error: 'Email already in use' });
    const passwordHash = await bcrypt.hash(password, 12);
    const user = await prisma.user.create({ data: { email, name: fullName, passwordHash, provider: 'email', status: 'ACTIVE' as any } as any });
...
router.post('/login-email', async (req: Request, res: Response) => {
  try {
    ...
    const user = await prisma.user.findFirst({ where: { email, NOT: { status: 'TERMINATED' as any } } as any });
