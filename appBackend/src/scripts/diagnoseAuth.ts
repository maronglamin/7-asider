...
async function main() {
  ...
  if (active === 0) {
    ...
    console.log('\n⚠ No email/password accounts — users must sign in with Google.');
  } else if (duplicateEmails.length > 0) {
    console.log('\n⚠ Duplicate emails detected — deploy latest auth fix and dedupe in Postgres.');
  } else {
    console.log('\n✓ User records look plausible.');
  }
}
