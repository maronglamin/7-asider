/** Book-facing copy when the field owner cannot receive Easypay yet (not linked, no wallets, etc.). */
...
/** Maps Easypay prepare API errors to short, non-technical text for the pay sheet. */
export function friendlyEasypayPrepareError(rawMessage: string | null | undefined): string {
  ...
  if (/Easypay payments are not configured on this server/i.test(m)) {
    return EASYPAY_SERVER_NOT_CONFIGURED;
  }
  if (
    /cannot accept Easypay payments yet/i.test(m) ||
    /Field owner is not linked to Easypay/i.test(m) ||
    /not linked to Easypay/i.test(m) ||
    /Link To EasyPay/i.test(m) ||
...
/** Wallet (Wave / Yonna) checkout — hide upstream Easypay HTTP noise and long JSON. */
export function friendlyEasypayWalletError(rawMessage: string | null | undefined): string {
  ...
  if (/Request failed:\s*5\d\d|failed:\s*5\d\d|Easypay\s+\w+\s+[^\n]+\s+5\d\d/i.test(m)) {
    return 'The payment service is temporarily unavailable. Please try again in a few moments.';
  }
  if (/Easypay|wallet|checkout|payment provider|Request failed/i.test(m)) {
    return 'We could not start this payment. Please try again in a moment or choose another method.';
  }
  return m;
}
